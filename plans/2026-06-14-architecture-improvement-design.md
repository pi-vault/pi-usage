...
- `src/providers/runtime.ts` — add 4 shared utilities (`fetchWithTimeout`, `readJsonObject`, `clampPercent`, `clampPercentRounded`)
...
- Unit tests for `fetchWithTimeout`, `readJsonObject`, `clampPercent`, `clampPercentRounded`
...
// Clamp to [0, 100] without rounding (opencode-go dashboard preserves fractional %)
export function clampPercent(value: number): number;

// Clamp to [0, 100] and round (minimax, stepfun integer percentages)
...
- All existing provider tests pass unchanged (109 tests across 11 files)
...
export function computeLabelWidth(
  columns: ColumnDef[],
  totalWidth: number,
): number;
```
...
export function computeCompatibility(snapshot: ProviderUsageSnapshot | null): {
  currentLiveProviderId: ProviderId | null;
  currentLiveProviderSnapshot: ProviderUsageSnapshot | null;
};

export function computeUsageField(snapshot: ProviderUsageSnapshot | null): {
  provider?: string;
